package com.jaydenxiao.common.ui.view.address;
...
public class ScrollerNumberPicker extends View {
     ...
     * 设置数据
     */
    private List<Cityinfo> dataList = new ArrayList<Cityinfo>();
    /**
     * 按下的坐标
     */
    private int downY;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        unitHeight = (int) attribute.getDimension(
                R.styleable.NumberPicker_picker_unitHight, 32);
        normalFont = attribute.getDimension(
                R.styleable.NumberPicker_picker_normalTextSize, 14.0f);
        selectedFont = attribute.getDimension(
                R.styleable.NumberPicker_picker_selecredTextSize, 18.0f);
        itemNumber = attribute.getInt(R.styleable.NumberPicker_picker_itemNumber, 7);
        normalColor = attribute.getColor(
                R.styleable.NumberPicker_picker_normalTextColor, 0xff000000);
        selectedColor = attribute.getColor(
                R.styleable.NumberPicker_picker_selecredTextColor, 0xffff0000);
        lineColor = attribute.getColor(R.styleable.NumberPicker_picker_lineColor,
                0xff000000);
        maskHight = attribute.getDimension(R.styleable.NumberPicker_picker_maskHight,
                48.0f);
        noEmpty = attribute.getBoolean(R.styleable.NumberPicker_picker_noEmpty, false);
        isEnable = attribute
                .getBoolean(R.styleable.NumberPicker_picker_isEnable, true);
        attribute.recycle();
     ...
     * @param index
     */
    public void moveToPostion(int index) {
        float move = itemList.get(index).moveToSelected();
        defaultMove((int) move);
        onSelectListener.endSelect(itemList.get(index).id, itemList.get(index).itemText);
    }
        ...
        public void drawSelf(Canvas canvas) {
            ...
            if (isSelected()) {
                ...
                moveToSelect = moveToSelect > 0 ? moveToSelect : moveToSelect
                        * (-1);
                // 计算当前字体大小
                float textSize = (float) (normalFont - (selectedFont - normalFont))
                        + ((float) (normalFont - (normalFont - (selectedFont - normalFont))) * (1.0f - (float) moveToSelect
                        / (float) unitHeight));
                float textlength = textPaint.measureText(itemText);
                if (textlength > controlWidth) {
                    textSize = textSize * controlWidth / textlength;
                }
                textPaint.setTextSize(textSize);
            } else {
                textPaint.setColor(normalColor);
                float textlength = textPaint.measureText(itemText);
                float newNormalFont = 0f;
                if (textlength > controlWidth) {
                    newNormalFont = normalFont * controlWidth / textlength;
                    textPaint.setTextSize(newNormalFont);
                }else {
                    textPaint.setTextSize(normalFont-(selectedFont - normalFont));
                }
            }
