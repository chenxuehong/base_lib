...
public class PullRecyclerViewGroup extends LinearLayout implements ViewTreeObserver.OnGlobalLayoutListener {
     ...
     * 滚动时间
     */
    private static final long ANIM_TIME = 400;//listview 或者recyclerview或者ScrollView
    private View childView;     // 用于记录正常的布局位置
    private Rect originalRect = new Rect();     //滚动时，移动的view和位置
    private List<View> mMoveViews = new ArrayList<View>();
    private List<Rect> mMoveRects = new ArrayList<Rect>();     // 在手指滑动的过程中记录是否移动了布局
    private boolean isMoved = false;     // 如果按下时不能上拉和下拉， 会在手指移动时更新为当前手指的Y值
    private float startY;     //阻尼
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        super.onLayout(changed, l, t, r, b);        //ScrollView中唯一的子控件的位置信息，这个位置在整个控件的生命周期中保持不变
    ...
    /**
     * 容器的的事件都在事件分发中处理，这里处理的是事件分发传递过来的事件，     *     * 传递过来的为RecyclerVIew的事件  不拦截，直接交给reyclerview处理     * @param ev     * @return
     */
    @Override
